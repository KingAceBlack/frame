...
type TotalLoadedFrame = number;
type KilledFoes = number;
let farcasterid: FarcasterID = '1';
let currentframe: CurrentFrame = 'battle3';
let deathFrame: DeathtFrame = '/page17';
let timerWatch: TimerWatch = 1717262784622;
let finalDecision: FinalDecision = 0
let totalLoadedFrame: TotalLoadedFrame = 50
let killedFoes: KilledFoes = 20
...
async function updateData(farcasterid: FarcasterID, currentframe: CurrentFrame, deathFrame: DeathtFrame, timerWatch: TimerWatch, finalDecision: FinalDecision, totalLoadedFrame: TotalLoadedFrame, killedFoes: KilledFoes) {
  ...
  const data = {
    ...
    loadedframes: player.framesLoaded,
...
async function updateData2(farcasterid: FarcasterID, currentframe: CurrentFrame, deathFrame: DeathtFrame, timerWatch: TimerWatch, finalDecision: FinalDecision) {
  // Construct the URL using template literals
  const url = `https://gpzytjchmkcglwzkxcrc.supabase.co/rest/v1/lorecast?fid=eq.${farcasterid}`;
   console.log(url);
  const data = {
    fid : farcasterid,
    lastknownframe: currentframe,
...
/*updateData(farcasterid, currentframe, deathFrame, timerWatch, totalLoadedFrame, killedFoes)
  .then(() => {
...
app.frame('/page1', async (c) => {
  ...
  try {
    ...
    if (item) {
      ...
      console.log('Farcaster health:', item.lastknownframe);
